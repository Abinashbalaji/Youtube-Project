...
def video_details(Video_ids):
    ...
    for items in Video_ids:
        ...
        data_1 = {'Video_id' : response_2['items'][0]['id'],
                ...
                'Duration' : response_2['items'][0]['contentDetails']['duration'],
                'Thumbnails' : response_2['items'][0]['snippet']['thumbnails'],
                'Tags' : response_2['items'][0]['snippet']['tags']
        }
...
def Channel_table():
    try:
        Create_query = ("CREATE TABLE Channel_data (Channel_id VARCHAR(40), Channel_Name VARCHAR(20),Published_at TIMESTAMP, Subsribers_count INT,Video_count INT, View_count INT, Description TEXT)")
    ...
    db = client.Youtube_project
    collection_1 = db['Harvest']
    for data in collection_1.find({"Channel_details.Channel_Name": "Vj Siddhu Vlogs"}):
        ch_list.append(data["Channel_details"])
        
    df=pd.DataFrame(ch_list) 

    for index,row in df.iterrows():
        Insert_query = ("INSERT INTO Channel_data (Channel_id, Channel_Name, Published_at,Subsribers_count, Video_count, View_count, Description) VALUES (%s,%s,%s,%s,%s,%s,%s)")
...
def Video_table():
    try:
        Create_query = ("CREATE TABLE Video_data (Video_id VARCHAR(15), Channel_id VARCHAR(30),Video_title VARCHAR(90), Video_description VARCHAR(250), Likes_count INT, Views_count INT, Comment_count INT, Favourite_count INT,Published_at TIMESTAMP, Duration INT, Thumbnails VARCHAR(250)")
    ...
    for index,row in df.iterrows():
        ...
        Insert_query = ("INSERT INTO Video_data (Video_id, Channel_id, Video_title,Video_description, Likes_count, Views_count, Comment_count,Favourite_count,Published_at,Duration,Thumbnails) VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)")
...
Selects = st.sidebar.radio("Navigation",["Home","Queries"])
